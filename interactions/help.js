  ...
  async interact(client, interaction) {
    try {
      ...
      const send = await interaction.reply({
        embeds: [makeEmbed(contentArray[counter])],
        ephemeral: true,
        components: [row_left],
        fetchReply: true,
      });
      ...
      collector.on("collect", async (i) => {
        ...
        await i.deferUpdate();
        if (i.customId === 'right') {
          if (counter !== contentArray.length - 2) {
