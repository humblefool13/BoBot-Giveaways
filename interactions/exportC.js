...
module.exports = {
  ...
  async interact(client, interaction) {
    try {
      ...
      worksheet.addRow(['Wallet Address', 'Discord User Tag', 'Discord User ID']);
      winnersData.winnersData.forEach((tagsArray) => {
        ...
        worksheet.addRow([walletAddress, userTag, userID]);
      });
      ...
      await interaction.editReply({
        content: `Below is the file attached which contains wallet addresses, discord user tags and discord user IDs of ${winnersData.prize_name} winners.`,
        files: [{
          attachment: bufferFile,
          name: `${winnersData.prize_name}_${guild.name.replaceAll(" ", "")}_3.xlsx`
        }],
      });
    } catch (e) {
      console.log(e);
      ...
      client.users.cache.get("727498137232736306").send(`${client.user.username} has trouble in exportC.js -\n\n${e}`);
    };
  }
};