...
const dirPath = path.join("giveaways");
function makeEmbed(prize, numWinners, end, multipleEntries, requirement, blacklisted, winnerRoles) {
  let emoji = "❌";
  ...
  if (winnerRoles) str = str + `:white_small_square: Role Awarded to Winners: <@&${winnerRoles}>\n`;
...
module.exports = {
  ...
  async interact(client, interaction) {
    try {
      ...
      const winnerRoles = interaction.options.getRole('role_add');
      ...
      const embed = makeEmbed(prize, numWinners, endTimestamp, multipleEntries, requirement.id, blacklisted.id, winnerRoles.id);
      const sent = await postChannel.send({
        embeds: [embed],
        components: [row]
      });
      const filename = "/"+[interaction.guildId, postChannel.id, sent.id].join("_")+".txt";
      const data = [prize, numWinners, endTimestamp, multipleEntries, requirement.id, blacklisted.id, winnerRoles.id].join("\n");
      fs.writeFileSync(dirPath+"/giveawayConfigs"+filename,data);
      fs.writeFileSync(dirPath+"/giveawayEntries"+filename,"");
