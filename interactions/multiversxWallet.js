...
module.exports = {
  ...
  async interact(client, interaction) {
    const modal = new ModalBuilder()
      .setTitle('Submit MultiversX Wallet Address')
      .setCustomId('modal');
    ...
    try {
      ...
      if (!find) {
        sent = await interaction.editReply({
          embeds: [
            MakeEmbedDes(
              'You have not saved any MultiversX wallet on this server, nor have you saved a global MultiversX wallet. Please click the “Submit new MultiversX wallet!” button below, copy and paste your wallet address in the pop-up to submit your wallet. Please remember that this wallet address will be automatically recorded when you win WL spots, so we highly recommend you submit a burner wallet.'
            ),
          ],
          components: [rownew],
          fetchReply: true,
        ...
        globalWallet = find.wallet_global_mulx;
        savedWallets = find.wallets_mulx;
        ...
        sent = await interaction.editReply({
          embeds: [
            MakeEmbedDes(
              `You have saved the following MultiversX wallet addresses:\n\nServer MultiversX Wallet: **${wallet}**\nGlobal MultiversX Wallet: **${
                globalWallet ? globalWallet : 'Not Saved Yet.'
              }**\n\nWould you like to change your wallet addresses? If so, please click the “Yes, change my MultiversX wallet!” copy and paste your new wallet address in the pop-up to submit your new wallet address. If you do not need to change your wallet addresses, please click “Dismiss message” at the bottom of the message.`
            ),
          ],
      ...
      collector.on('collect', async (i) => {
        ...
        if (walletNew.includes(' '))
          return i.editReply({
            embeds: [
              MakeEmbedDes(
                `Please enter a valid MultiversX address. The currently entered one ( **${walletNew}** ) includes a space \` \`.`
              ),
            ],
            components: [],
          });
        ...
        if (!find) {
          await new wallets({
            discord_id: interaction.user.id,
            wallet_global_mulx: 'Not Submitted Yet.',
            wallets_mulx: [[interaction.guild.id, walletNew]],
          })
            .save()
            .catch((e) => {});
        } else {
          ...
          find.wallets_mulx = savedWallets;
        ...
        if (wallet === 'Not Saved Yet.') {
          ...
          sentv2 = await interaction.editReply({
            embeds: [
              MakeEmbedDes(
                `You have successfully changed your MultiversX wallet address for this server!\n\n**Removed: ${wallet}\nNew: ${walletNew}**\n\nEverytime you win a MultiversX WL giveaway, this wallet will be automatically recorded!\n\nPlease also consider setting it as global MultiversX wallet if not done so yet:\nA global wallet is the wallet address that the bot will remember for all discord servers that use SnapBot. The advantage of saving a wallet address as a global wallet is that you will not have to submit your wallet again in all discord servers that use SnapBot. SnapBot will use your global wallet in those servers **UNLESS** you specifically set a server wallet for a particular discord server.\n\nIf you do not need to set your global wallet, please click “Dismiss message” at the bottom of the message.`
              ),
            ],
            components: [rowGlobal],
          });
        ...
        globalcollector.on('collect', async (i) => {
          ...
          findNew.wallet_global_mulx = walletNew;
          await findNew.save().catch((e) => {});
          await i.update({
            components: [rowGlobalDis],
            embeds: [
              MakeEmbedDes(
                `Global MultiversX wallet address:\n\n**${walletNew}**\n\nThis is now set as your global MultiversX wallet and will be used for all discord servers that use SnapBot **UNLESS** you specifically set a server wallet for a particular discord server.`
              ),
            ],
          });
        });
