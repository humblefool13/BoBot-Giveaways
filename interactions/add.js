    ...
    try {
      ...
      if (!find_sub && !find_config) {
        ...
        return interaction.editReply({
          content: `The server with server ID : ${server} has been successfully subscribed on <t:${parseInt(interaction.createdTimestamp / 1000)}:F> for ${months} months and the subscription will end on <t:${parseInt((interaction.createdTimestamp + months * 31 * 24 * 60 * 60 * 1000) / 1000)}:F>.`
        }).catch((e) => {
          console.log(e);
        });
      } else if (find_sub) {
        const endTimestampOld = find_sub.end_timestamp;
        find_sub.end_timestamp = endTimestampOld + months * 31 * 24 * 60 * 60 * 1000;
        find_sub.save().then(() => {
          return interaction.editReply({
            content: `The subscription for the server with server ID : ${server} which was supposed to end on <t:${parseInt(endTimestampOld / 1000)}:F> is now extended to <t:${parseInt((endTimestampOld + months * 31 * 24 * 60 * 60 * 1000) / 1000)}:F>.`,
          }).catch((e) => {
            console.log(e);
          });
        }).catch((e) => { });
        ...
        return interaction.editReply({
          content: `The subscription for the server with server ID : ${server} which had expired, is renewed on <t:${parseInt(interaction.createdTimestamp / 1000)}:F> for ${months} months and the subscription will end on <t:${parseInt((interaction.createdTimestamp + months * 31 * 24 * 60 * 60 * 1000) / 1000)}:F>.`
        }).catch((e) => {
          console.log(e);
        });
      };
