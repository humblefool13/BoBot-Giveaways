    ...
    try {
      ...
      collector.on("collect", async (i) => {
        ...
        if (walletNew.includes(" ")) return i.editReply({
          embeds: [MakeEmbedDes(`Please enter a valid Solana address. The currently entered one ( **${walletNew}** ) includes a space \` \`.`)],
          components: [],
        });
        let wrong = false;
        try {
        ...
        if (wrong) {
          return i.editReply({
            embeds: [MakeEmbedDes(`Please enter a valid Solana address. The currently entered one ( **${walletNew}** ) is invalid.`)],
            components: [],
          });
        };
        let sentv2;
