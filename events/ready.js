  ...
  async execute(client) {
    ...
    async function endGiveaways() {
      ...
      giveawaysConfigsDir.forEach(async (file) => {
        ...
        let masterArray = [["Discord ID", "Discord Tag", "Submitted Wallet"].join("\t")];
        winners.forEach((winner) => {
          const member = members.find((m) => m.id === winner) ? members.find((m) => m.id === winner) : { user: { tag: "Not Found" } };
          const wallet = wallets.find((saved) => saved.discord_id === winner) ? wallets.find((saved) => saved.discord_id === winner) : { wallet: "Not Submitted" };
          masterArray.push([winner, member.user.tag, wallet.wallet].join("\t"));
        });
        let exportString = masterArray.join("\n");
        // STUCK HERE
        await fs.appendFile('export.xls', exportString, (err) => {
