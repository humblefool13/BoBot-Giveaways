    ...
    if (userAccount.discord_id === userId) {
      ...
      if (chain === 'Ethereum') {
        ...
        if (find) {
          ...
          return userAccount.wallet_global_eth
            ? userAccount.wallet_global_eth
            : 'Not Found.';
        }
        ...
        if (find) {
          ...
          return userAccount.wallet_global_sol
            ? userAccount.wallet_global_sol
            : 'Not Found.';
        }
      } else if (chain === 'Aptos') {
        ...
        if (find) {
          ...
          return userAccount.wallet_global_apt
            ? userAccount.wallet_global_apt
            : 'Not Found.';
        }
        ...
        if (find) {
          ...
          return userAccount.wallet_global_mulx
            ? userAccount.wallet_global_mulx
            : 'Not Found.';
        }
      }
    }
...
const rowExport = new ActionRowBuilder().addComponents(
  new ButtonBuilder()
    .setLabel('Format:')
  ...
  async execute(client) {
    ...
    async function endGiveaways() {
      ...
      for (const file of giveawaysConfigsDir) {
        if (!file.includes('processing')) {
          const fileData1 = fs.readFileSync(
            `./giveaways/giveawayConfigs/${file}`,
          ...
          if (Date.now() >= endTimestamp) {
            ...
            fs.rename(
              `./giveaways/giveawayConfigs/${file}`,
              `./giveaways/giveawayConfigs/processing-${file}`,
              (e) => {
                if (e) console.log(e);
              }
            );
            const entries1 = fs.readFileSync(
              `./giveaways/giveawayEntries/${file}`,
              { encoding: 'utf8', flag: 'r' }
            );
            ...
            if (entries.length === 1 && entries[0] === '') {
              ...
              do {
                ...
                if (
                  !winners.includes(entries[index]) &&
              ...
              const mintTimestamp = parseTimestamp(
                mintTime,
                config.server_timezone.toLowerCase()
              );
              ...
              if (message && channel) {
                ...
                const winnerChannel = await client.guilds.cache
                  .get(location[0])
                  .channels.fetch(config.winners_channel)
                  .catch((e) => {});
                let sent;
                  ...
                  await postChannel.send({
                    embeds: [
                      new EmbedBuilder()
                        ...
                        .setColor('#8A45FF')
                        .setFooter({ text: exportID }),
                    ],
                    components: [rowExport, messageLinkRow],
                  });
                  if (winnerRole !== 'NA') {
                    for (let winner in winners) {
                      let winnerUser = await guild.members.fetch(winners[winner]);
    ...
    async function sendReminders() {
      ...
      winners_data.forEach(async (giveawayWinnerData) => {
        ...
        if (reminderTimestamp <= Date.now()) {
          ...
          const winnerChannel = await client.guilds.cache
            .get(giveawayWinnerData.guild_id)
            .channels.fetch(configs.reminders_channel);
    ...
    async function checkSubs() {
      ...
      subs.forEach(async (sub) => {
        ...
        const config = await config_records
          .findOne({
            server_id: sub.server_id,
          })
          .catch((e) => {
